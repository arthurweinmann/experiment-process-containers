...
pub fn writev(fd: RawFd, iov: &[IoVec<&[u8]>]) -> Result<usize, Errno> {
    let res = unsafe { libc::writev(fd, iov.as_ptr() as *const libc::iovec, iov.len() as c_int) };

    Errno::result(res).map(|r| r as usize)
}

pub fn readv(fd: RawFd, iov: &mut [IoVec<&mut [u8]>]) -> Result<usize, Errno> {
...
pub fn pwritev(fd: RawFd, iov: &[IoVec<&[u8]>],
               offset: off_t) -> Result<usize, Errno> {
    let res = unsafe {
...
pub fn pwrite(fd: RawFd, buf: &[u8], offset: off_t) -> Result<usize, Errno> {
    let res = unsafe {
...
pub fn pread(fd: RawFd, buf: &mut [u8], offset: off_t) -> Result<usize, Errno>{
    let res = unsafe {
...
#[cfg(target_os = "linux")]
pub fn process_vm_writev(pid: super::unistd::Pid, local_iov: &[IoVec<&[u8]>], remote_iov: &[RemoteIoVec]) -> Result<usize, Errno> {
    let res = unsafe {
...
pub fn process_vm_readv(pid: super::unistd::Pid, local_iov: &[IoVec<&mut [u8]>], remote_iov: &[RemoteIoVec]) -> Result<usize, Errno> {
    let res = unsafe {
