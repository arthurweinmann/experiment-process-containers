...
/*
 * check out O_ASYNC OFlag that closes the file descriptor once an `execve` call is made.
 *
...
pub fn open<P: ?Sized + NixPath>(
    ...
    let fd = path.with_nix_path(|cstr| unsafe { libc::open(cstr.as_ptr(), oflag, mode) })?;

    Errno::result(fd)
}

pub fn open_oflag<P: ?Sized + NixPath>(path: &P, oflag: OFlag, mode: Mode) -> Result<RawFd, Errno> {
...
pub fn open_no_mode<P: ?Sized + NixPath>(
    path: &P,
    oflag: libc::c_int,
) -> Result<libc::c_int, Errno> {
    let fd = path.with_nix_path(|cstr| unsafe { libc::open(cstr.as_ptr(), oflag) })?;
...
pub fn openat_no_mode<P: ?Sized + NixPath>(
    dirfd: RawFd,
    path: &P,
    oflag: OFlag,
) -> Result<RawFd, Errno> {
    let fd =
        path.with_nix_path(|cstr| unsafe { libc::openat(dirfd, cstr.as_ptr(), oflag.bits()) })?;
    Errno::result(fd)
}

pub fn openat_no_mode_cstr(dirfd: RawFd, path: &CStr, oflag: libc::c_int) -> Result<RawFd, Errno> {
    let fd = unsafe { libc::openat(dirfd, path.as_ptr(), oflag) };
    Errno::result(fd)
}
...
pub fn rename_cstr(old: &CStr, new: &CStr) -> Result<(), Errno> {
    let res = unsafe { libc::rename(old.as_ptr(), new.as_ptr()) };
    Errno::result(res).map(drop)
}

fn wrap_readlink_result(v: &mut Vec<u8>, res: ssize_t) -> Result<OsString, Errno> {
...
pub fn readlink<P: ?Sized + NixPath>(path: &P) -> Result<OsString, Errno> {
...
pub fn readlinkat<P: ?Sized + NixPath>(dirfd: RawFd, path: &P) -> Result<OsString, Errno> {
    let mut v = Vec::with_capacity(libc::PATH_MAX as usize);
    let res = path.with_nix_path(|cstr| unsafe {
        libc::readlinkat(
            dirfd,
...
pub fn fcntl(fd: RawFd, arg: FcntlArg) -> Result<c_int, Errno> {
    let res = unsafe {
        match arg {
...
pub fn flock(fd: RawFd, arg: FlockArg) -> Result<(), Errno> {
    use self::FlockArg::*;
...
#[cfg(any(target_os = "linux", target_os = "android"))]
pub fn tee(fd_in: RawFd, fd_out: RawFd, len: usize, flags: SpliceFFlags) -> Result<usize, Errno> {
    let ret = unsafe { libc::tee(fd_in, fd_out, len, flags.bits()) };
    Errno::result(ret).map(|r| r as usize)
}

#[cfg(any(target_os = "linux", target_os = "android"))]
pub fn vmsplice(fd: RawFd, iov: &[IoVec<&[u8]>], flags: SpliceFFlags) -> Result<usize, Errno> {
    let ret = unsafe {
        libc::vmsplice(
            fd,
...
pub fn posix_fallocate(fd: RawFd, offset: libc::off_t, len: libc::off_t) -> Result<(), Errno> {
