...
use super::fcntl::{at_rawfd, AtFlags};
...
pub fn umask_mode(mode: libc::mode_t) -> Mode {
    let prev = unsafe { libc::umask(mode) };
    Mode::from_bits(prev).expect("[BUG] umask returned invalid Mode")
}

pub fn stat<P: ?Sized + NixPath>(path: &P) -> Result<FileStat, Errno> {
...
pub fn lstat<P: ?Sized + NixPath>(path: &P) -> Result<FileStat, Errno> {
    let mut dst = mem::MaybeUninit::uninit();
...
pub fn fstat(fd: RawFd) -> Result<FileStat, Errno> {
    let mut dst = mem::MaybeUninit::uninit();
...
pub fn fchmod(fd: RawFd, mode: Mode) -> Result<(), Errno> {
...
#[inline]
pub fn futimens(fd: RawFd, atime: &TimeSpec, mtime: &TimeSpec) -> Result<(), Errno> {
...
pub fn mkdirat<P: ?Sized + NixPath>(fd: RawFd, path: &P, mode: Mode) -> Result<(), Errno> {
    let res = path
